...
def sell_gap_resistance(df, idx):
    """
    低开缺口：T-1 收盘 > T 日最高价（全日落在昨收之下）；T 日低开。
    自 T 日起至昨收，历史最高价从未达到过 T 日最高价（缺口下沿未被有效越过）；
    当前切入缺口上沿附近且走弱时触发。仅 V3 计入卖出。
    """
    ...
    for T in range(idx - 5, max(1, idx - 200), -1):
        if T < 1:
            break
        prev_c = float(close[T - 1])
        t_high = float(high[T])
        if prev_c <= t_high * (1.0 + 1e-9):
            continue
        if open_[T] >= prev_c * (1.0 - 1e-9):
            continue
        if idx <= T + 5:
            continue
        # T+1 .. idx-1 日内最高价均未达到 T 日最高价（缺口下沿未被越过）
        if T + 1 < idx:
            mx = float(np.max(high[T + 1 : idx]))
            if mx >= t_high * (1.0 - 1e-9):
                continue
        # 当日已接近或进入缺口区域（昨收为缺口上沿）
